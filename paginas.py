...
class Pages:
    ...
    def __init__(self, ctx, *, entries, per_page=12, show_entry_count=True):
        ...
        if not self.permissions.embed_links:
            raise CannotPaginate('Bot não tem permissão de linkar embeds.')

        if not self.permissions.send_messages:
            raise CannotPaginate('Bot não pode mandar mensagens.')

        if self.paginating:
            # verify we can actually use the pagination session
            if not self.permissions.add_reactions:
                raise CannotPaginate('Bot não tem permissão de adicionar botões.')

            if not self.permissions.read_message_history:
                raise CannotPaginate('Bot não tem permissão de ler o histórico de mensagens.')
    ...
    def prepare_embed(self, entries, page, *, first=False):
        ...
        if self.paginating and first:
            p.append('')
            p.append('Confuso? clique no botão \N{INFORMATION SOURCE} para mais informação.')
    ...
    async def numbered_page(self):
        """Permite digitar o número da página que deseja ir"""
        to_delete = []
        to_delete.append(await self.channel.send('Para que página você deseja ir?'))
    ...
    async def show_help(self):
        """shows this message"""
        messages = ['Bem vindo ao paginador interativo!\n']
        messages.append('Isso permite a navegação por páginas de texto ' \
        ...
        embed.clear_fields()
        embed.description = '\n'.join(messages)
        embed.set_footer(text=f'Nós estávamos na página {self.current_page} antes dessa mensagem.')
    ...
    async def paginate(self):
        ...
        while self.paginating:
            try:
                payload = await self.bot.wait_for('raw_reaction_add', check=self.react_check, timeout=120.0) #tempo até usuário interagir
            except asyncio.TimeoutError:
                self.paginating = False
                try:
